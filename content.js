...
function createProgressWidget() {
  ...
  // (Styling is the same as before, so it's omitted for brevity here)
...
function updateProgress(message, progressPercentage, isError = false) {
  ...
  if (progressBar) {
    ...
    progressBar.style.backgroundColor = isError ? "#ffc107" : "#007bff"; // Use yellow for non-halting error
  }
}

function showSummaryReport(successes, failures) {
    // Remove the progress widget to replace it with the report
    ...
    report.innerHTML = `
        <h3 style="margin: 0 0 15px 0; padding-bottom: 10px; border-bottom: 1px solid #eee;">Campaign Finished</h3>
        <h4 style="margin: 10px 0 5px 0;">✅ Successful Sends (${successes.length})</h4>
        <ul style="list-style-type: none; padding: 0; margin: 0 0 15px 0; max-height: 150px; overflow-y: auto; border: 1px solid #eee; padding: 5px;">${successItems}</ul>
        <h4 style="margin: 10px 0 5px 0;">❌ Failed Sends (${failures.length})</h4>
        <ul style="list-style-type: none; padding: 0; margin: 0 0 20px 0; max-height: 150px; overflow-y: auto; border: 1px solid #eee; padding: 5px;">${failureItems}</ul>
        <button id="close-summary-btn" style="width: 100%; padding: 10px; background-color: #007bff; color: white; border: none; border-radius: 5px; cursor: pointer;">Close</button>
    `;
...
window.addEventListener("trigger-sending", async () => {
  ...
  chrome.storage.local.get(['contactList'], async (result) => {
    if (chrome.runtime.lastError || !result.contactList) {
      updateProgress("Error: Could not load contacts.", 0, true);
    ...
    for (let i = 0; i < contactsToSend.length; i++) {
      ...
      try {
        ...
        // Update the contact's 'sent' status in the full list
        const originalContact = fullContactList.find(c => c.number === contact.number && c.message === contact.message);
        if (originalContact) originalContact.sent = true;

        // Save the updated list back to storage after each success
        ...
        // Add the error message to the contact object for reporting
        contact.error = error.message; 
        failedToSend.push(contact);

        await sleep(3000); // Wait 3 seconds as requested before continuing
      }
    }

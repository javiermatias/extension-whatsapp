...
function createProgressWidget() { /* ...your existing code... */ }
function updateProgress(message, progressPercentage, isError = false) { /* ...your existing code... */ }
function showSummaryReport(successes, failures) { /* ...your existing code... */ }
...
// =====================================================================
// --- MODIFIED: Main Sending Logic with Integrated Checks ---
// =====================================================================
async function startSendingProcess() {
  ...
  // Note: Wrapped the chrome.storage.local.get in a Promise for cleaner async/await
  const { contactList } = await new Promise(resolve => chrome.storage.local.get('contactList', resolve));

  try {
    if (!contactList) throw new Error("Could not load contacts.");

    const contactsToSend = contactList.filter(contact => !contact.sent);
    ...
    for (let i = 0; i < contactsToSend.length; i++) {
      ...
      try {
        ...
        await new Promise(resolve => chrome.storage.local.set({ 'contactList': contactList }, resolve));
      } catch (error) {
...
// --- MODIFIED: Core sendMessage Function ---
...
// --- Paste your full, original UI functions and sendMessage function here ---
